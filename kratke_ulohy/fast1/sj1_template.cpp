...
void Gauss_BS( const float * __restrict__  inA, const float * __restrict__  inB, float * __restrict__ outX, const int tn, const int tm) {
  int i,i2,j,l=0,k=0;
  float s;
  //const float * pa;
  //const float * pb;
//  omp_set_num_threads (4);
//  #pragma omp parallel for private(i,l,j,s, i2) schedule (runtime)
  for(k=0;k<tm;k++){
/*    outX[(tn-1)*tm+k] = inB[(tn-1)*tm+k]/inA[(tn-1)*tn+tn-1];
    for(i=tn-2;i>=0;i--){
      s = 0;
      for(j=i;j<tn;j++){
	s += outX[j*tm+k] * inA[i*tn+j]; 
      }
      outX[(i)*tm+k] = (inB[i*tm+k]-s)/inA[i*tn+i];
    }
*/
    for(i=tn-1;i>=0;i--){
      //i2=(tn-1)-i; 
